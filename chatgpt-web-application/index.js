...
app.post("/signout", async (req, res) => {
  const {sessionId,id} = req.body;
    ...
    const time=new Date().toISOString().slice(0, 19).replace('T', ' ');
    const content= JSON.stringify (await redisclient.get(id)).replace(/'/g, "\\'")
  const sql=`UPDATE Session SET SessionEndTime = '${time}', content = '${content}' WHERE sessionId=${sessionId};`;
  con.query(sql, (err, rows) => {
      if (err){
        con.end()
        ...
        con.end()
          return res.status(200).send({message: "session endded"});
...
app.post("/register", async (req, res) => {
    ...
    const clientldap = ldap.createClient({
      ...
      clientldap.bind('cn='+process.env.LDAP_USER+',dc=nodomain', process.env.LDAP_PASSWORD, function(err) {
        if (err) {
      ...
      clientldap.add('uid='+email+',ou=user,dc=nodomain', entry, function(err) {
        if (err) {
          clientldap.destroy();
